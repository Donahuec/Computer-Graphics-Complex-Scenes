...
Implements functions to work with GL and normal meshes
*/
...
struct meshGLMesh {
	GLuint triNum, vertNum, attrNum, attrDim, vaoNum;
	GLuint *attrDims;
	GLuint buffers[2];
	GLuint *vaos;
};
...
void meshGLVAOInitialize(meshGLMesh *meshGL, GLuint index, GLint attrLocs[]){
	...
	for(int i=0;i<meshGL->attrNum;i++){
		attrTotal+=meshGL->attrDims[i];
	}

	glGenVertexArrays(1, &meshGL->vaos[index]);
	...
	for(int i=0;i<meshGL->attrNum;i++){
		...
		glVertexAttribPointer(attrLocs[i], meshGL->attrDims[i], GL_DOUBLE, GL_FALSE, 
			attrTotal * sizeof(GLdouble), BUFFER_OFFSET(attrCurr * sizeof(GLdouble)));
		attrCurr+=meshGL->attrDims[i];
	}
...
/* Renders the already-initialized OpenGL mesh, using the VAO */

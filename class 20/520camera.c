...
void camRender(camCamera *cam, GLint viewingLoc) {
	...
	if(cam->projectionType == camORTHOGRAPHIC) {
		mat44Orthographic(cam->projection[0], cam->projection[1], cam->projection[2], 
		cam->projection[3], cam->projection[4], cam->projection[5], proj);
	} else {
		mat44Perspective(cam->projection[0], cam->projection[1], cam->projection[2], 
		cam->projection[3], cam->projection[4], cam->projection[5], proj);
	}

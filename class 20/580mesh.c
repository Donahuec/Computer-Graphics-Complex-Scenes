...
struct meshGLMesh {
	GLuint triNum, vertNum, attrDim, attrNum, vaoNum;
	GLuint *attrDims;
	GLuint *vaos;
	GLuint buffers[2];
};
...
void meshGLVAOInitialize(meshGLMesh *meshGL, GLuint index, GLint attrLocs[]) {
	...
	for (int i = 0; i < meshGL->attrNum; i += 1) {
		...
		glVertexAttribPointer(attrLocs[i], meshGL->attrDims[i], GL_DOUBLE, GL_FALSE, 
			stride * sizeof(GLdouble), BUFFER_OFFSET(bufOffset));
			bufOffset += meshGL->attrDims[i] * sizeof(GLdouble);
	}
...
void meshGLDestroy(meshGLMesh *meshGL) {
	...
	for (int i = 0; i < meshGL->vaoNum; i += 1) {
